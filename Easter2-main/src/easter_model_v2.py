...
def Easter2_2():
    ...
    y_pred = tensorflow.keras.layers.Activation('sigmoid', name="Final")(data)
...
def train():
    ...
    training_data = data_loader(config.DATA_PATH_V2, config.BATCH_SIZE)
    validation_data = data_loader(config.DATA_PATH_V2, config.BATCH_SIZE)
    test_data = data_loader(config.DATA_PATH_V2, config.BATCH_SIZE)
